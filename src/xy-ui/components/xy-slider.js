...
export default class XySlider extends HTMLElement {
  ...
  constructor() {
    ...
    shadowRoot.innerHTML = `
        ...
        <xy-tips id='slider-con' dir=${this.vertical ? 'right' : 'top'} style="--percent:${
      (this.defaultvalue - this.min) / (this.max - this.min)
    }" tips="${this.showtips && !this.disabled ? this.defaultvalue : ''}" suffix="${this.suffix}" prefix="${
      this.prefix
    }"><input id='slider' value=${this.defaultvalue} min=${this.min} max=${this.max} step=${this.step} ${
