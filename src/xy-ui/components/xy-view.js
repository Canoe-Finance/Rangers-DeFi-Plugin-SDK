      ...
      this.addEventListener('dragend', ev => {
        if (this.cloneObj) {
          ...
          const reset = this.cloneObj.animate(
            [{ transform: this.cloneObj.style.transform }, { transform: `translate3d( ${left}px ,${top}px,0)` }],
            {
              duration: 200,
              easing: 'ease-in-out',
            },
    ...
    if (this.resizable) {
      ...
      document.addEventListener('mousemove', ev => {
        if (this.resizing) {
          ...
          this.style.transform = `translate3d(${Number(transforms[0]) + offsetX}px,${
            Number(transforms[1]) + offsetY
          }px,0)`
          this.dispatchEvent(
