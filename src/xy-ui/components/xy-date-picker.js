            ...
            <div class="date-con" data-type="date">
                <div class="date-mode date-date">
                    ...
                    <div class="date-body">
                        ${Array.from(
                          { length: 42 },
                          el => '<button class="date-button date-day-item" type="flat"></button>',
                        ).join('')}
                    </div>
                </div>
                ...
                <div class="date-mode date-year">
                    ${Array.from(
                      { length: 20 },
                      el => '<button class="date-button date-year-item" type="flat"></button>',
                    ).join('')}
                </div>
            </div>
        </div>
    ...
    switch (this.mode) {
        ...
        this.days.forEach((el, i) => {
          ...
          if (this.range) {
            ...
            const disabled =
              (this.range === 'left' &&
                right &&
                parseDate(el.dataset.date, 'month') > parseDate(right.value, 'month')) ||
      ...
      case 'month':
        this.months.forEach((el, i) => {
          ...
          if (this.minormax) {
            el.disabled =
              el.dataset.date < parseDate(this.min, 'month') || el.dataset.date > parseDate(this.max, 'month')
          } else {
            el.disabled = false
          }
          if (this.range) {
  ...
  constructor() {
    ...
    shadowRoot.innerHTML = `
        ...
        <xy-popover class="date-picker" id="popover" ${this.dir ? "dir='" + this.dir + "'" : ''}>
            ...
            }><span id="datetxt"></span><svg class="icon" viewBox="0 0 1024 1024"><path d="M880 184H712v-64c0-4.4-3.6-8-8-8h-56c-4.4 0-8 3.6-8 8v64H384v-64c0-4.4-3.6-8-8-8h-56c-4.4 0-8 3.6-8 8v64H144c-17.7 0-32 14.3-32 32v664c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V216c0-17.7-14.3-32-32-32z m-40 656H184V460h656v380zM184 392V256h128v48c0 4.4 3.6 8 8 8h56c4.4 0 8-3.6 8-8v-48h256v48c0 4.4 3.6 8 8 8h56c4.4 0 8-3.6 8-8v-48h128v136H184z" p-id="8054"></path></svg></xy-button>
