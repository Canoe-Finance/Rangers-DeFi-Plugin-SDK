...
export class Autocomplete implements ComponentInterface {
   ...
   */
  @Prop() placeholder = 'Search name or paste address'
  ...
  handleKeyDown(e: KeyboardEvent) {
    if (['ArrowDown', 'ArrowUp', 'Down', 'Up'].indexOf(e.key) >= 0) {
      // some older browsers use Up/Down instead of ArrayUp/ArrowDown (https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key/Key_Values)
      e.preventDefault()
  ...
  handleKeyUp(key, text) {
    if (['ArrowDown', 'ArrowUp', 'Enter', 'Tab', 'Escape'].indexOf(key) < 0) {
      // IE doesn't have Array.includes (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/includes)
          ...
          <div class={this.cssClasses.suggestions}>
            {this.data.map((suggestion, index) => {
              return (
                ...
                >
                  <img class="w-[14px] h-[14px] mr-[5px]" src={suggestion.logoURI} />
